    ...
    */
    #afficher_les_donnees(){
        ...
        if(this.#obj_webs.type_de_requete==='update' ){
            ...
            cmd+='interface1.module1(';
            cmd+='  chemin_module1(\'_js/c_requete_sql1.js\'),';
            cmd+='  methode3(maj_ne_pas_traiter_la_maj_ts_modification),';
            cmd+='  parametre3(';
        ...
        if(__gi1.chi_id_projet==='1'){
            ...
            t+='<input style="display:none;" id="vv_sur_base_de_reference" type="checkbox" checked="false" />';
        }
        t+='</div>';
        t+='<table>';
            ...
            if(this.#obj_webs.conditions){
                ...
                if(this.#obj_webs.conditions.length === 0){
                    ...
                    for( i=0 ; i < this.#obj_webs.conditions.length ; i++ ){
                        ...
                        cmd+=' )';
                        cmd+=')';
                        t+='<div class="hug_bouton" id="retirer_ce_champ_de_where" style="max-width:90%;overflow-wrap:anywhere;"  data-hug_click="' + cmd + '">' + this.#obj_webs.conditions[i].formule.replace( /</g , '&lt;' ).replace( />/g , '&gt;' ).replace( /"/g , '&quot;' ) + '</div>';
                    }
                }
            }else{
                var cmd='';
                    ...
                    for( var l=1 ; l < l01 ; l++ ){
                        if(tab[l][7] === 0){
                            ...
                            tableau_des_champs_en_sortie.push( {
                                    ...
                                    "champ_en_bdd"  : this.#obj_init['bases'][id_numerique_base_principale].tables[nom_de_la_table].champs[nom_du_champ]
                                } );
                        }
                    }
                        ...
                        if("champs" === tab[j][1] && tab[j][2] === 'f'){
                            for( let k=j + 1 ; k < l01 ; k=tab[k][12] ){
                                if(tab[k][1] === 'champ' && tab[k][2] === 'f'){
                                    ...
                                    this.#obj_init['bases'][ind]['tables'][nom_de_la_table]['champs'][le_champ.nom_du_champ]=le_champ;
                                }
                            }
                        }
                    }
