        ...
        if(element.indexTypeKw){
            ...
            if(obj1.__xst === __xsu){
                if('UNIQUE' === obj1.__xva){
                    ...
                    return(this.#astsql_le( {"__xst" : __xer ,"__xme" : this.__m_rev1.nl2() + ' AFR '} ));
                }
            }else{
                return(this.#astsql_le( {"__xst" : __xer ,"__xme" : this.__m_rev1.nl2() + ' '} ));
            }
                    ...
                    if(element.items[i].name){
                        ...
                        switch (name.toLowerCase()){
                            ...
                            case 'min' : name='min';
                                break;
                            default:
                                t+='AFR("' + name + '" ' + this.__m_rev1.nl2() + ')';
                                break;
                                
                        }
                        t+=name + args;
                    }else{
                        t+='AFR("' + element.items[i].type + '" ' + this.__m_rev1.nl2() + ')';
                    }
                    break;
    ...
    */
    #traite_binary_expr( element , niveau , parent , options , vient_de ){
        ...
        switch (operator.toUpperCase()){
            ...
            default:
                debugger;
                t+='AFR( "' + operator + '" ' + this.__m_rev1.nl2() + ')';
                break;
    ...
    */
    #traite_specification( element , niveau , parent , options , vient_de ){
        ...
        switch (element.type){
            ...
            default: t+='AFR(' + element.type + ' ' + this.__m_rev1.nl2() + ')';
                break;
        }
        return({"__xst" : __xsu ,"__xva" : t});
    }
    ...
    #traite_func_args( element , niveau , parent , options , vient_de ){
        ...
        */
        if(element.distinctKw
            ...
            return(this.#astsql_le( {"__xst" : __xsu ,"__xme" : this.__m_rev1.nl2() + ' afr'} ));
        }
        if(element.args){
        ...
        switch (element.type){
            ...
            case 'join_expr' :
                ...
                switch (parent.type){
                    case 'select_stmt' :
                        if(element.hasOwnProperty( 'operator' ) && Array.isArray( element.operator )){
                            if(element.operator.length === 2
                                ...
                                debugger;
                                t+='AFR(#traite_expr' + ' ' + this.__m_rev1.nl2() + ')';
                            }
                    ...
                    default:
                        debugger;
                        t+='AFR(#traite_expr_parent "' + parent.type + '" ' + this.__m_rev1.nl2() + ')';
                        break;
                ...
                switch (parent.type){
                    case 'join_expr' :
                        if(vient_de.length - 2 >= 0 && vient_de[vient_de.length - 2] === 'from_clause'){
                            ...
                            t+='AFR(#traite_expr_parent-type "' + parent.type + '" ' + this.__m_rev1.nl2() + ')';
                            break;
                        }
                        break;
    ...
    #convertit_select( element , niveau , parent , options , vient_de ){
        ...
        for(let i in element.clauses){
            switch (element.clauses[i].type){
                ...
                default:
                    t+='AFR(' + element.clauses[i].type + ' ' + this.__m_rev1.nl2() + ')';
                    break;
