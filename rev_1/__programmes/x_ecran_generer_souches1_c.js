...
class x_ecran_generer_souches1{
    ...
    */
    constructor( mat , d , __ig1 ){
        ...
        this.#_developpement1=new _developpement1( mat , d , this.__ig1 );
    }
    /*
      =============================================================================================================
    */
    ...
    charger_les_requetes_souches( mat , d , le_colis1=null ){
        if(le_colis1 && le_colis1.__xst === __xsu){
            for(let req in le_colis1.__xva.requetes){
                ...
                /*  */
                this.#a_construire[elt['T0_chp_type_requete']]={"id" : parseInt( elt['T0_chi_id_requete'] , 10 ) ,"type" : elt['T0_chp_type_requete']};
            }
        }
        let o1='';
        o1+='<table border="1">';
    ...
    */
    selectionner_une_table( mat , d ){
        ...
        if(zone_select !== ''){
            ...
            let nom_de_la_table=document.getElementById( 'vv_les_tables' ).value;
            if(nom_de_la_table !== ''){
                window.location.hash='m1(n1(x_ecran_generer_souches1),f1(entree_module(chi_id_basedd(' + chi_id_basedd + '),nom_de_la_table(\'' + this.__ig1.fi1( nom_de_la_table ) + '\'))))';
                this.charger_base1( chi_id_basedd , nom_de_la_table );
            }else{
                window.location.hash='m1(n1(x_ecran_generer_souches1),f1(entree_module(chi_id_basedd(' + chi_id_basedd + '))))';
            }
        }else{
            window.location.hash='m1(n1(x_ecran_generer_souches1),f1(entree_module(chi_id_basedd(' + chi_id_basedd + '))))';
            this.__ig1.ajoute_les_evenements_aux_boutons( null );
    ...
    recuperer_zone_travail_pour_les_bases2( mat , d , le_colis1=null ){
        ...
        for(let i in le_colis1.__xva.les_bases_du_projet){
            if(le_colis1.__xva.les_bases_du_projet[i]['T0_chp_rev_travail_basedd'] === null){
                ...
                this.#arbre[le_colis1.__xva.les_bases_du_projet[i]['T0_chi_id_basedd']]={"matrice" : null ,"tables" : []};
                        ...
                        if(mat2[j][1] === 'créer_table'){
                            for( let k=j + 1 ; k < l02 ; k=mat2[k][12] ){
                                if(mat2[k][1] === 'nom_de_la_table' && mat2[k][2] === 'f' && mat2[k][8] === 1 && mat2[k + 1][2] === 'c'){
                                    this.#arbre[le_colis1.__xva.les_bases_du_projet[i]['T0_chi_id_basedd']].tables.push( mat2[k + 1][1] );
                                }
                            }
                        }
                    }
