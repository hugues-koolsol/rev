                        ...
                        if(tableau_de_javascripts_a_convertir.length > 0){
                            ...
                            for( var indjs=0 ; indjs < tableau_de_javascripts_a_convertir.length ; indjs++ ){
                                try{
                                    ...
                                    let js_moins_commentaires_sql=tableau_de_javascripts_a_convertir[indjs].__xva.replace( /&amp;/g , '&' );
                                    let regex=/\/\*sql_inclure_deb[\s\S]*?sql_inclure_fin\*\//g;
                                    js_moins_commentaires_sql=js_moins_commentaires_sql.replace( regex , '' );
                                    var obj0=parseur_javascript.parse( js_moins_commentaires_sql , {"ecmaVersion" : 'latest' ,"sourceType" : 'module' ,"ranges" : true ,"onComment" : tableau_des_commentaires_js} );
                                }catch(e){
...
export{w_html_vers_rev1 as w_html_vers_rev1};