...
class _tri_arbre1{
    ...
    #options={
        ...
        "decallage_entre_niveaux_en_px" : /* entre 5 et 30 */30 ,
        "boutons_du_menu" : [] ,
        "class_du_bouton_menu" : '' ,
        "border_zone_principale" : '2px purple inset' ,
        "hauteur_separateur_en_px" : /* entre 10 et 30 */10 ,
    ...
    */
    constructor( par_id_de_l_element , options ){
        ...
        if(lst1.length > 0){
            console.error( 'v_tri_arbre1 id="' + par_id_de_l_element + '" déjà défini :-/' );
            return;
        }
        this.#racine_html=document.getElementById( par_id_de_l_element );
        if(this.#racine_html.tagName.toUpperCase() !== 'UL'){
            console.error( 'v_tri_arbre1 la racine doit être un <ul>' );
            return;
        }
        if( typeof options !== 'object'){
            console.error( 'v_tri_arbre1 les options doivent être représentées sous forme d\'objet' );
            return;
        }
    ...
    */
    #doigt_bas( e ){
        ...
        for( let i=0 ; i < lst1.length ; i++ ){
            ...
            lst1[i].style.touchAction='none';
        }
        document.getElementById( this.#id_div ).style.overflowY='hidden';
        document.getElementById( this.#id_div ).style.touchAction='none';
        e.stopPropagation();
        e.preventDefault();
    ...
    */
    #doigt_haut( e ){
        ...
        document.body.touchAction='';
        /* console.log(e.changedTouches[0]) */
        let lst1=document.querySelectorAll( 'dialog' );
        for( let i=0 ; i < lst1.length ; i++ ){
            lst1[i].style.overflowY='scroll';
            lst1[i].style.touchAction='';
        }
        try{
            document.getElementById( this.#id_div ).style.overflowY='scroll';
            document.getElementById( this.#id_div ).style.touchAction='';
        } catch {}
        e.stopPropagation();
    ...
    construit_html_de_arbre( id_interne_parent , niveau=0 ){
        ...
        for( let i=0 ; i < this.arbre.length ; i++ ){
            if(this.arbre[i].id_interne_parent === id_interne_parent){
                ...
                */
                ...
                t+=' data-separateur="' + this.arbre[i].separateur + '"';
                t+='>';
                t+='  <div';
                if(this.arbre[i].separateur === '1' || this.arbre[i].separateur === 1){
                    t+=' data-separateur="' + this.arbre[i].separateur + '"';
                    t+='    style="text-align:left;flex-grow:1;color:hotpink;" ';
                }else{
                ...
                if(this.#options.triable === false){
                    le_html+='<div data-poignee_pour_tri="' + this.arbre[i].id_interne + '" style="float:right;visibility:hidden;min-width:1em;" class="' + this.#options.class_du_bouton_deplacer + '">↕</div>';
                }else{
                    le_html+='<div data-poignee_pour_tri="' + this.arbre[i].id_interne + '" style="float:right;min-width:1em;" class="' + this.#options.class_du_bouton_deplacer + '">↕</div>';
                }
                le_html+='</div>';
                /*
        ...
        if(id_interne_parent === 0){
            ...
            */
            ...
            for( let i=0 ; i < lst1.length ; i++ ){
                lst1[i].addEventListener( 'mousedown' , this.#souris_bas_listener , false );
                lst1[i].addEventListener( 'touchstart' , this.#doigt_bas_listener , false );
            }
            /*
              =============================================================================================
            ...
            */
            if(this.#options.arborescent === 1){
                ...
                for( let i=0 ; i < lst2.length ; i++ ){
                    lst2[i].addEventListener( 'mousedown' , this.#souris_bas_listener , false );
                }
            }
            /*
              =============================================================================================
                ...
                for( let i=0 ; i < this.#options.boutons_du_menu.length ; i++ ){
                    ...
                    try{
                        document.getElementById( id ).addEventListener( 'mousedown' , ( e ) => {
                                if(this.#options.boutons_du_menu[i]['fonction'] === undefined){
                                    let id='menu_' + this.cle_aleatoire + '_' + i;
        ...
        for( let i=0 ; i < lst.length ; i++ ){
            ...
            if(elem.nodeName.toUpperCase() === 'LI' || elem.nodeName.toUpperCase() === 'UL'){
                if(elem.nodeName.toUpperCase() === 'LI'){
                    ...
                    let separateur=0;
                    if(elem.getAttribute( 'data-separateur' ) && elem.getAttribute( 'data-separateur' ) === '1'){
                        separateur=1;
                    }
                    this.arbre.push( {
                            ...
                            "contenu" : /* "nodeName" : elem.nodeName , */'' ,
