    ...
    */
    async executer_sql3( mat , d , donnees_recues , donnees_retournees , options_generales ){
        ...
        */
        ...
        try{
            this.__gi1.ma_trace1( 'Ecécution de la requête' );
            let b=await db1temp.exec( donnees_recues[__xva]['source_sql'] );
            await db1temp.close();
            this.__gi1.__xsi[__xsu].push( 'la base ' + id_bdd_de_la_base + ' a bien été modifiée contexte(' + contexte + ')' );
        }catch(e){
            this.__gi1.ma_trace1( 'bug dans la requête' );
            await db1temp.close();
            this.__gi1.__xsi[__xer].push( ' erreur d\'exécution de la requête de la base ' + chemin_bdd + ' <pre>' + donnees_recues[__xva]['source_sql'] + '</pre> [' + this.__gi1.nl2( e ) );
            if(donnees_recues[__xva]['source_sql'].indexOf( 'DROP' ) >= 0
                   && donnees_recues[__xva]['source_sql'].indexOf( 'COLUMN' ) >= 0
            ){
