...
class __fnt1{
        ...
        for(let i in les_tests){
            ...
            if(le_test.nt === 'non_vide1'){
                if(fo1[le_test.nz] === ''){
                    ...
                    try{
                        document.getElementById( le_test.nz ).focus();
                    } catch {}
                    return({"__xst" : __xer});
                }
                ...
                if(!(fo1[le_test.nz] === 0 || fo1[le_test.nz] === 1)){
                    this.__ig1.ajoute_message( {"__xst" : __xer ,"__xme" : 'la valeur pour "' + le_test.lib + '" doit 0 ou 1'} );
                    this.__ig1.affiche_les_messages();
                    this.__ig1.retablir_les_boutons_masques();
                    try{
                        document.getElementById( le_test.nz ).focus();
                ...
                if(!(fo1[le_test.nz] === null || fo1[le_test.nz] === 0 || fo1[le_test.nz] === 1)){
                    this.__ig1.ajoute_message( {"__xst" : __xer ,"__xme" : 'la valeur pour "' + le_test.lib + '" doit 0 ou 1 ou vide'} );
                    this.__ig1.affiche_les_messages();
                    this.__ig1.retablir_les_boutons_masques();
                    try{
                        document.getElementById( le_test.nz ).focus();
    ...
    action_maj_nulle_zero_un( mat , d ){
        ...
        for( let i=d + 1 ; i < l01 ; i=mat[i][12] ){
            if('vv_id' === mat[i][1] && mat[i][2] === 'f' && mat[i][8] === 1 && mat[i + 1][2] === 'c'){
                vv_id=mat[i + 1][1];
            }else if('vv_libelle' === mat[i][1] && mat[i][2] === 'f' && mat[i][8] === 1 && mat[i + 1][2] === 'c'){
                vv_libelle=mat[i + 1][1];
    ...
    */
    html_edition_de_zones_autrex2( tup , les_donnees_du_champ , that ){
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            o1+='        </div>';
            o1+=this.__ig1.lien_parent2( les_donnees_du_champ.nom_du_lien ,  /* 'grandeurs4' , */ les_donnees_du_champ.nom_du_champ , les_donnees_du_champ.nom_du_champ + '_libelle' , that.moi ,  /* chi_id_parametre */ 0 ,  /* libelle_du_champ */ '' ,  /* vider */ les_donnees_du_champ.bouton_vider );
            o1+='        </div>';
    ...
    */
    html_edition_de_date2( tup , les_donnees_du_champ ){
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            o1+=' autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" class="yy_input1" ';
            o1+=' />';
            o1+=this.__ig1.__fnt1.boutons_date1( les_donnees_du_champ.nom_du_champ );
            o1+='          <div><i style="text-align:left;font-weight:100;">';
            if(les_donnees_du_champ.description_du_champ){
                o1+=les_donnees_du_champ.description_du_champ.replace( /¶LF¶/g , '<br />' );
            }
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            if(les_donnees_du_champ.hasOwnProperty('les_suggestions') && les_donnees_du_champ.les_suggestions.length > 0){
                ...
                for(let i = 0 ; i<les_donnees_du_champ.les_suggestions.length;i++){
                    ...
                    o1+='id(' + les_donnees_du_champ.nom_du_champ + '),';
                    o1+='valeur(valeur_constante(\'' + tt + '\'))';
                    o1+=')))">' + les_donnees_du_champ.les_suggestions[i] + '</div>';
                }
                o1+='</div>';
            }
    ...
    */
    ...
    /*
      =============================================================================================================
    */
    html_de_zones_float2( tup , les_donnees_du_champ ){
        let o1='<!-- html_de_zones_float2 -->';
        o1+='<div class="yy_edition_champ1">';
        o1+='    <div class="yy_edition_libelle1">';
        o1+='        <div>' + les_donnees_du_champ.libelle_du_champ + '</div>';
        o1+='    </div>';
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            o1+=' style="';
            o1+='height: var(--t_hauteur_input1);';
            o1+='width: ' + (this.__ig1.css_dimensions.t_de_num * 21 + 2 * this.__ig1.css_dimensions.t_pad_inp + 2 * this.__ig1.css_dimensions.t_input_border) + 'px;';
            o1+='text-align:right';
            o1+='"';
            o1+=' size="22"';
            o1+=' maxlength="22"';
            /* lors de d'un refresh, le "tup" est le_colis */
            if(tup && !tup.hasOwnProperty( '_CA_' )){
                o1+=' value="' + tup['T0_' + les_donnees_du_champ.nom_du_champ]['toLocaleString']( undefined ) + '"';
            }else{
                if(les_donnees_du_champ.__contexte === 'creer1'
                       && les_donnees_du_champ.hasOwnProperty( 'valeur_par_defaut' )
                       && les_donnees_du_champ.valeur_par_defaut !== null
    ...
    /*
      =============================================================================================================
    */
    html_de_zones_decimale2( tup , les_donnees_du_champ ){
        let o1='<!-- html_de_zones_decimale2 -->';
        o1+='<div class="yy_edition_champ1">';
        o1+='    <div class="yy_edition_libelle1">';
        o1+='        <div>' + les_donnees_du_champ.libelle_du_champ + '</div>';
        o1+='    </div>';
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            o1+='text-align:right';
            o1+='"';
            o1+=' size="' + (les_donnees_du_champ.lng_size + plus_espaces) + '"';
    ...
    */
    html_edition_de_zones_textarea2( tup , les_donnees_du_champ ){
        ...
        if(les_donnees_du_champ.__contexte === 'supprimer1' || les_donnees_du_champ.__contexte === 'voir1'){
            ...
            if(les_donnees_du_champ.hasOwnProperty( 'format_du_source' ) && les_donnees_du_champ.format_du_source === 2){
            ...
            }else if(les_donnees_du_champ.hasOwnProperty( 'format_du_source' ) && les_donnees_du_champ.format_du_source === 1){
    ...
    */
    html_de_zones_entier1( tup , les_donnees_du_champ){ 
        ...
        if(les_donnees_du_champ.libelle_du_champ !== null){
            ...
            o1+='        <div style="margin:auto 3px;">' + les_donnees_du_champ.libelle_du_champ + ' :&nbsp;</div>';
            o1+='    </div>';
        }
        if(les_donnees_du_champ.__contexte === 'modifier1' || les_donnees_du_champ.__contexte === 'creer1'){
            o1+='      <input';
            o1+=' id="' + les_donnees_du_champ.nom_du_champ + '"';
            o1+=' value="' + this.__ig1.fi2( tup['T0_' + les_donnees_du_champ.nom_du_champ] ) + '" ';
            ...
            o1+=' class="yy_input1"';
            o1+=' size="' + les_donnees_du_champ.lng_size + '"';
            o1+=' maxlength="' + les_donnees_du_champ.lng_maxlength + '" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"';
            o1+=' />';
        }else{
            o1+=this.__ig1.fi2( tup['T0_' + les_donnees_du_champ.nom_du_champ] );
        }
    ...
    */
    html_de_zones_decimale1( nom_du_champ , enreg , contexte , libelle=null , lng_size , lng_maxlength ){
        ...
        if(libelle !== null){
            ...
            o1+='        <div style="margin:auto 3px;">' + libelle + ' :&nbsp;</div>';
            o1+='    </div>';
        }
        if(contexte === 'modifier1'){
            o1+='      <input';
            ...
            o1+=' type="text" class="yy_input1" size="' + lng_size + '" maxlength="' + lng_maxlength + '" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"';
            o1+='  />';
        }else{
    ...
    */
    html_de_zones_zero_un_pour_ecran1( nom_du_champ , enreg , contexte , libelle=null ){
        let o1='';
        if(libelle !== null){
            o1+='<label for="' + nom_du_champ + '_libelle" style="display:inline-flex;margin-left:3px;padding:var(--t_padding);border: var(--t_border) #669900 solid;margin:3px;">';
            o1+='    <div id="' + nom_du_champ + '_libelle" style="display:flex;">';
            o1+='        <div style="margin:auto 3px;">' + libelle + ' :&nbsp;</div>';
            o1+='    </div>';
        }
    ...
    */
    html_de_zones_nulle_zero_un_pour_ecran( nom_du_champ , enreg , contexte , libelle=null ){
        ...
        o1+='vv_libelle(' + nom_du_champ + '_libelle),';
        o1+='valeur(),';
    ...
    */
    lsto( mat , d , x ){
        ...
        let max=10;
        let step=1;
        let font_size_fixe='16px';
        let largeur_des_champs_input='3rem';
        let style_des_tds0='border:1px black solid;text-align:center;font-size:' + font_size_fixe + ';';
        let style_des_tds1=' style="border:1px black solid;font-size:' + font_size_fixe + ';"';
        let style_des_tds2=' style="border:1px green solid;height:40px;font-size:' + font_size_fixe + ';"';
        ...
        for(let i in this.__ig1.reference_stockage_local['aspect']){
            let tta=this.__ig1.reference_stockage_local['aspect'][i];
            let vva=this.__ig1.stockage_local['aspect'][i];
            t+='<table style="width:360px;"><tr>';
            t+='<td style="' + style_des_tds0 + ';height:46px;width:60%;padding:5px;">';
            t+='' + tta.libelle0 + ' ';
            t+='<input type="range" id="' + i + '" min="' + tta.min + '" max="' + tta.max + '" step="' + tta.step + '" ';
            t+=' value="' + vva.valeur + '" ';
            t+=' data-rev_change_ou_entree="m1(n1(__ig1),f1(maj_contenu(';
            t+=' type_cible(inner_html)';
            t+=' id(' + i + '_nouvelle),';
            t+=' valeur(valeur_de_champ(' + i + '))';
            t+='))),';
            if(tta.nom_du_style !== ''){
                ...
                t+=' id(' + i + '_nouvelle),';
                t+=' valeur(';
                t+='   nom_du_style(' + tta.nom_du_style + ')';
                t+='   valeur_de_champ(' + i + '),';
                t+='   dimension(' + vva.dimension + ')';
                t+=' )';
                t+=')))';
            }
            t+='"';
            ...
            t+='))';
            t+='"';
            if(tta.style_interface){
                t+=' style="' + tta.style_interface + '"';
            }
            t+='></td>';
            /*
            */
            t+='<td style="' + style_des_tds0 + ';height:36px;width:40%;';
            if(i === '--t_marge_hb'){
                t+='vertical-align:top;';
            }
            t+='">';
            t+='  <div id="' + i + '_nouvelle" style="display:content;border-color:red;border-style:solid;min-width:' + largeur_des_champs_input + ';';
            if(i === '--t_rayon_b'){
                t+='border-radius:' + vva.valeur + 'px;';
            }
            if(i === '--t_padding'){
                t+='padding:' + vva.valeur + 'px;';
            }else{
                t+='min-height:26px;';
            }
            if(i === '--t_police'){
                t+='font-size:' + vva.valeur + 'px;';
            }else{
                t+='font-size:16px;';
            }
            if(i === '--t_border'){
                t+='border-width:' + vva.valeur + 'px;';
            }else if(i === '--t_input_border'){
                t+='border-width:' + vva.valeur + 'px;';
            }else{
                t+='border-width:1px;';
            }
            if(i === '--t_marge_gd'){
                t+='margin:auto ' + vva.valeur + 'px;';
            }
            if(i === '--t_marge_hb'){
                t+='margin:' + vva.valeur + 'px auto;';
            }
            t+='">' + vva.valeur + '</div>';
            t+='</td></tr></table>';
        }
        for(let i in this.__ig1.reference_stockage_local['parametres']){
            let tta=this.__ig1.reference_stockage_local['parametres'][i];
            let vva=this.__ig1.stockage_local['parametres'][i];
            t+='<table style="width:360px;"><tr>';
            if(i === '__deverminage'){
                if(vva.valeur === 0){
                    t+='<td style="' + style_des_tds0 + ';background:SpringGreen;color:black;min-width:70%;">';
                }else if(vva.valeur === 1){
                    t+='<td style="' + style_des_tds0 + ';background:DarkOrange;color:black;min-width:70%;">';
                }else if(vva.valeur === 2){
            ...
            t+='' + tta.libelle0 + ' ';
            t+='<input type="range" id="' + i + '" min="' + tta.min + '" max="' + tta.max + '" step="' + tta.step + '" ';
            t+=' value="' + vva.valeur + '" data-rev_change_ou_entree="m1(n1(__ig1)f1(maj_contenu(';
            t+='   type_cible(inner_html)';
            t+='   id(' + i + '_nouvelle),';
            t+='   valeur(valeur_de_champ(' + i + '))';
            t+='  ))),';
            ...
            */
            t+='<td style="' + style_des_tds0 + ';height:36px;width:40%;">';
            t+='  <div id="' + i + '_nouvelle" style="display:content;border-width:1px;border-color:red;border-style:solid;min-width:' + largeur_des_champs_input + ';';
            t+='">' + vva.valeur + '</div>';
            t+='</td></tr></table>';
        }
