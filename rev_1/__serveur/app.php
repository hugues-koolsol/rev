        ...
        */
        ...
        if(substr($donnees_recues[__x_action],0,34) === 'recuperation_de_la_premiere_page()'){
            ...
            */
            ...
            $a_chercher=substr($donnees_retournees[__xva]['redirection'],0,strpos($donnees_retournees[__xva]['redirection'],'('));
            foreach($donnees_retournees[__xbo] as $k1=>$v1){
             if(strpos($v1,$a_chercher)!==false){
              //indice_menu afr
              // il faut "allumer" le bouton du menu
              $donnees_retournees[__x_signaux][__xdv][]='<pre>' . enti1(var_export($a_chercher , true )) .'</pre><pre>' . enti1(var_export($v1 , true )) .'</pre> [' . __LINE__ . ']';
