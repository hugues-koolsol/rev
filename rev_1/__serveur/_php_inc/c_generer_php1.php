    ...
    */
    function page_editer1(&$donnees_retournees,/*matrice*/&$mat,&$donnees_recues){
        ...
        for( $i=1 ; $i < count($mat) ; $i++ ){
            ...
            if($mat[$i][1] === 'c_generer_php1.page_editer1'){
        ...
        $o1='';
        $o1.='<h1>générer le php sur '.$nom_de_la_table.'(' . $chi_id_basedd .')</h1>';
        $o1.='<div id="contenu_generer_le_php"></div>';

        $cmd='';
        $cmd.='maj_interface1(
         charger_module1(
          chemin_module1("_js/c_php_bdd1.js"),
          initialisation1(
           (chi_id_basedd,' . $chi_id_basedd . '),
           (nom_de_la_table,\'' . $nom_de_la_table . '\'),
           (module_js1,c_php_bdd1.js),
           (nom_module1,c_php_bdd1)
