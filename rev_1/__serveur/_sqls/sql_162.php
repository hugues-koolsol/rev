<?php
function sql_162($par,&$donnees_retournees,$that){
    $champs0='
    ...
    $from0='
      FROM `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.tbl_sources T0
       LEFT JOIN `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.tbl_projets T1 ON T1.chi_id_projet = T0.chx_projet_id_source

       LEFT JOIN `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.tbl_dossiers T2 ON T2.chi_id_dossier = T0.chx_dossier_id_source
    ';
    $sql0.=$from0;
    $where0=' WHERE ( /* */ `T0`.`chi_id_source` = '.sq1($par['T0_chi_id_source']).' AND `T0`.`chx_projet_id_source` = '.sq1($par['T0_chx_projet_id_source']).')'.PHP_EOL;
    $sql0.=$where0;
    $donnees0=array();
