<?php
function sql_66($par,&$donnees_retournees,$that,$db){
    $sql0='
      SELECT 
            `T0`.`chp_nom_dossier` , `T0`.`chx_parent_dossier`
                  FROM tbl_dossiers T0
            WHERE (`T0`.`chi_id_dossier` = '.sq1($par['T0_chi_id_dossier']).' AND `T0`.`chx_projet_dossier` = '.sq1($par['T0_chx_projet_dossier']).')
            ;
    ';
