<?php
function sql_158($par,&$donnees_retournees,$that){
    $sql0='
      /*meta(tester_les_dependances_dans_le_php(1))*/
      
      DELETE FROM `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.tbl_dossiers
          WHERE (`chi_id_dossier` = '.sq1($par['chi_id_dossier']).' AND `chx_projet_dossier` = '.sq1($par['chx_projet_dossier']).') ;
    ';
    ...
    if($dep>0){
        $donnees_retournees[__x_signaux][__xer][]=' erreur lors de la suppression, il existe des dépendances sql_158() [' . __LINE__ . ']';
        return array(
            __xst => __xer,
            'sql0' => $sql0,
