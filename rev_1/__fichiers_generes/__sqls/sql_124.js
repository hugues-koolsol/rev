    ...
    */
    async sql( par , donnees_retournees ){
        ...
        if(par.hasOwnProperty( 'T0_chi_id_metier' ) && par['T0_chi_id_metier'] !== ''){
            where0+='\r\n' + this.__gi1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chi_id_metier`' , par['T0_chi_id_metier'] );
        }
        if(par.hasOwnProperty( 'T0_chp_nom_metier' ) && par['T0_chp_nom_metier'] !== ''){
            where0+=` AND \`T0\`.\`chp_nom_metier\` LIKE ` + this.__gi1.__fnt1.sq2( par['T0_chp_nom_metier'] ) + '\r\n';
        }
        sql0+=where0;
        const order0=`
        ...
        sql0+=plage0;
        /* this.__gi1.ma_trace1('sql_124 sql0=',sql0); */
        let lignes=[];
        try{
            ...
            return(this.__gi1.traite_erreur_sql( 124 , e , sql0 , donnees_retournees , {} ));
        }
        /*  */
