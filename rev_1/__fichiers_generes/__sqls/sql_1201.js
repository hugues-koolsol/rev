    ...
    */
    async sql( tup ){
        ...
        try{
            ...
            if(tup.hasOwnProperty( 'T0_chp_cle_grandeur' ) && tup.T0_chp_cle_grandeur !== ''){
                where0+=` AND \`T0\`.\`chp_cle_grandeur\` LIKE ` + this.__ig1.__fnt1.sq2( tup.T0_chp_cle_grandeur , 'T0_chp_cle_grandeur' ) + '\r\n';
            }
            if(tup.hasOwnProperty( 'T0_chi_id_grandeur' ) && tup.T0_chi_id_grandeur !== ''){
                if(tup.T0_chi_id_grandeur === 0){
                    where0+=' AND `T0`.`chi_id_grandeur` IS NULL \r\n';
                }else{
                    where0+='\r\n' + this.__ig1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chi_id_grandeur`' , tup.T0_chi_id_grandeur );
                }
            }
            if(tup.hasOwnProperty( 'T0_cht_rev_grandeur' ) && tup.T0_cht_rev_grandeur !== ''){
                where0+=` AND \`T0\`.\`cht_rev_grandeur\` LIKE ` + this.__ig1.__fnt1.sq2( tup.T0_cht_rev_grandeur , 'T0_cht_rev_grandeur' ) + '\r\n';
            }
            if(tup.hasOwnProperty( 'T0_che_actif_grandeur' ) && tup.T0_che_actif_grandeur !== ''){
                where0+=` AND \`T0\`.\`che_actif_grandeur\` = ` + this.__ig1.__fnt1.sq1( tup.T0_che_actif_grandeur , 'T0_che_actif_grandeur' ) + `` + '\r\n';
            }
        }catch(e){
        ...
        for(let numero_de_ligne in lignes){
            donnees0.push( {
                    ...
                    "T0_chx_parametre_grandeur" : lignes[numero_de_ligne][3] ,
                    "T0_che_actif_grandeur" : lignes[numero_de_ligne][4] ,
                    "T1_chp_cle_parametre" : lignes[numero_de_ligne][5] ,
                    "T1_chp_nom_parametre" : lignes[numero_de_ligne][6] ,
                    "T1_cht_rev_parametre" : (lignes[numero_de_ligne][7]===null?null:lignes[numero_de_ligne][7].substr(0,200)) ,
                    "T1_cht_ordre_parametre" : (lignes[numero_de_ligne][8]===null?null:lignes[numero_de_ligne][8].substr(0,200)) ,
                    "T1_che_pour_admin_parametre" : lignes[numero_de_ligne][9]
                } );
        }
        /* comptage */
