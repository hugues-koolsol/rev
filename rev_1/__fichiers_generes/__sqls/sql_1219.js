...
class sql_1219{
    ...
    */
    async sql( tup ){
        ...
        try{
            if(tup.hasOwnProperty( 'T0_chi_id_acces' ) && tup.T0_chi_id_acces !== ''){
                if(tup.T0_chi_id_acces === 0){
                    where0+=' AND `T0`.`chi_id_acces` IS NULL \r\n';
                }else{
                    where0+='\r\n' + this.__ig1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chi_id_acces`' , tup.T0_chi_id_acces );
                }
            }
            if(tup.hasOwnProperty( 'T0_chp_nom_acces' ) && tup.T0_chp_nom_acces !== ''){
                where0+=` AND \`T0\`.\`chp_nom_acces\` LIKE ` + this.__ig1.__fnt1.sq2( tup.T0_chp_nom_acces , 'T0_chp_nom_acces' ) + '\r\n';
            }
            if(tup.hasOwnProperty( 'T0_che_actif_acces' ) && tup.T0_che_actif_acces !== ''){
                where0+=` AND \`T0\`.\`che_actif_acces\` = ` + this.__ig1.__fnt1.sq1( tup.T0_che_actif_acces , 'T0_che_actif_acces' ) + `` + '\r\n';
            }
            if(tup.hasOwnProperty( 'T0_chx_groupe_acces' ) && tup.T0_chx_groupe_acces !== ''){
                if(tup.T0_chx_groupe_acces === 0){
                    where0+=' AND `T0`.`chx_groupe_acces` IS NULL \r\n';
                }else{
                    where0+='\r\n' + this.__ig1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chx_groupe_acces`' , tup.T0_chx_groupe_acces );
                }
            }
            ...
            if(tup.hasOwnProperty( 'T0_chx_metier_acces' ) && tup.T0_chx_metier_acces !== ''){
                if(tup.T0_chx_metier_acces === 0){
                    where0+=' AND `T0`.`chx_metier_acces` IS NULL \r\n';
                }else{
                    where0+='\r\n' + this.__ig1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chx_metier_acces`' , tup.T0_chx_metier_acces );
                }
            }
            if(tup.hasOwnProperty( 'T2_chp_nom_metier' ) && tup.T2_chp_nom_metier !== ''){
                where0+=` AND \`T2\`.\`chp_nom_metier\` LIKE ` + this.__ig1.__fnt1.sq2( tup.T2_chp_nom_metier , 'T2_chp_nom_metier' ) + '\r\n';
            }
            if(tup.hasOwnProperty( 'acces_pas_dans' ) && tup.acces_pas_dans !== ''){
                where0+=` AND \`T0\`.\`chi_id_acces\` NOT IN ` + tup.acces_pas_dans + '\r\n';
            }
        }catch(e){
        ...
        sql0+=plage0;
        /* this.__ig1.ma_trace1('sql_1219 sql0=',sql0); */
        let lignes=[];
        try{
            ...
            return(this.__ig1.traite_erreur_sql( 1219 , e , sql0 , {} ));
        }
        /*  */
