const __xer=0;
const __xsu=1;
const __xal=2;
const __xif=3;
const __xdv=4;
const __xst='__xst';
const __xsi='__xsi';
class sql_1372{
    /*
      =============================================================================================================
    */
    async sql( par ){
        const champs0=`
          \`T0\`.\`chi_id_basedd\` , \`T0\`.\`chp_rev_travail_basedd\` , \`T0\`.\`chp_commentaire_basedd\` , \`T0\`.\`chp_fournisseur_basedd\`
        `;
        let sql0='SELECT ' + champs0;
        const from0=`
          FROM  tbl_bdds T0        `;
        sql0+=from0;
        /* ATTENTION : pas de condition dans cette liste */
        const where0=' WHERE 1 ';
        sql0+=where0;
        /* this.__ig1.ma_trace1('sql_1372 sql0=',sql0); */
        let lignes=[];
        try{
            let statement=await this.__db1.prepare( sql0 );
            lignes=await statement.values();
            await statement.finalize();
        }catch(e){
            return(this.__ig1.traite_erreur_sql( 1372 , e , sql0 , {} ));
        }
        let donnees0=[];
        for(let col of lignes){
            donnees0.push( {
                    "T0_chi_id_basedd" : col[0] ,
                    "T0_chp_rev_travail_basedd" : col[1] ,
                    "T0_chp_commentaire_basedd" : col[2] ,
                    "T0_chp_fournisseur_basedd" : col[3]
                } );
        }
        return({"__xst" : __xsu ,"__xva" : donnees0 ,"sql0" : sql0 ,"where0" : where0});
    }
    /*
      =============================================================================================================
    */
    moi='sql_1372';
    __ig1=null;
    __db1=null;
    /*
      =============================================================================================================
    */
    constructor( __ig1 , __db1 ){
        this.__ig1=__ig1;
        this.__db1=__db1;
    }
}
export{sql_1372 as sql_1372};