...
class sql_1153{
    ...
    */
    async sql( tup ){
        ...
        `;
        sql0+=from0;
        const where0=` WHERE ( \`T2\`.\`chi_id_acces\` = ` + this.__ig1.__fnt1.sq1( tup.T2_chi_id_acces , 'T2_chi_id_acces' ) + `
         AND T1.chi_id_autorisation = T0.chx_autorisation_menu)`;
        sql0+=where0;
        /* this.__ig1.ma_trace1('sql_1153 sql0=',sql0); */
        let lignes=[];
        try{
            ...
            return(this.__ig1.traite_erreur_sql( 1153 , e , sql0 , {} ));
        }
        let donnees0=[];
        for(let col of lignes){
            donnees0.push( {
                    "T1_chx_source_autorisation" : col[0] ,
                    "T0_chp_titre_menu" : col[1] ,
                    "T0_chp_methode_menu" : col[2] ,
                    "T3_chp_nom_source" : col[3] ,
                    "T0_cht_libelle_menu" : col[4] ,
                    "T0_cht_initialisation_menu" : col[5] ,
                    "T0_chi_id_menu" : col[6] ,
                    "T0_cht_condition_menu" : col[7] ,
                    "T0_cht_condition_js_menu" : col[8] ,
                    "T0_chx_autorisation_menu" : col[9] ,
                    "T3_cht_condition_js_source" : col[10]
                } );
        }
