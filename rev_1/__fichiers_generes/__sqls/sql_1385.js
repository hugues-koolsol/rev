const __xer=0;
const __xsu=1;
const __xal=2;
const __xif=3;
const __xdv=4;
const __xst='__xst';
const __xsi='__xsi';
class sql_1385{
    /*
      =============================================================================================================
    */
    async sql( tup ){
        const champs0=`
          \`T0\`.\`chi_id_requete\` , \`T0\`.\`cht_commentaire_requete\` , \`T0\`.\`chp_type_requete\` , \`T0\`.\`cht_rev_requete\` , \`T0\`.\`cht_sql_requete\` , 
          \`T0\`.\`cht_matrice_requete\` , \`T0\`.\`che_est_souche_requete\` , \`T0\`.\`chp_table_reference_requete\` , \`T0\`.\`che_base_reference_requete\`
        `;
        let sql0='SELECT ' + champs0;
        const from0=`
          FROM  tbl_requetes T0        `;
        sql0+=from0;
        const where0=` WHERE \`T0\`.\`chi_id_requete\` < ` + this.__ig1.__fnt1.sq1( tup.nb_max , 'nb_max' ) + ``;
        sql0+=where0;
    const order0=`
       ORDER BY  \`T0\`.\`chi_id_requete\` ASC`;
    sql0+=order0;
    const plage0='';
    sql0+=plage0;
        /* this.__ig1.ma_trace1('sql_1385 sql0=',sql0); */
        let lignes=[];
        try{
            let statement=await this.__db1.prepare( sql0 );
            lignes=await statement.values();
            await statement.finalize();
        }catch(e){
            return(this.__ig1.traite_erreur_sql( 1385 , e , sql0 , {} ));
        }
        let donnees0=[];
        for(let col of lignes){
            donnees0.push( {
                    "T0_chi_id_requete" : col[0] ,
                    "T0_cht_commentaire_requete" : col[1] ,
                    "T0_chp_type_requete" : col[2] ,
                    "T0_cht_rev_requete" : col[3] ,
                    "T0_cht_sql_requete" : col[4] ,
                    "T0_cht_matrice_requete" : col[5] ,
                    "T0_che_est_souche_requete" : col[6] ,
                    "T0_chp_table_reference_requete" : col[7] ,
                    "T0_che_base_reference_requete" : col[8]
                } );
        }
        return({"__xst" : __xsu ,"__xva" : donnees0 ,"sql0" : sql0 ,"where0" : where0});
    }
    /*
      =============================================================================================================
    */
    moi='sql_1385';
    __ig1=null;
    __db1=null;
    /*
      =============================================================================================================
    */
    constructor( __ig1 , __db1 ){
        this.__ig1=__ig1;
        this.__db1=__db1;
    }
}
export{sql_1385 as sql_1385};