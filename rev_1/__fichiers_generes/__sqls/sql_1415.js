...
class sql_1415{
    ...
    */
    async sql( tup ){
        ...
        if(tup.n_chi_id_source === null || tup.n_chi_id_source === ''){
        ...
        */
        ...
        let tableau_champs=[];
        try{
            if(tup.n_chi_id_source === undefined || tup.n_chi_id_source === '' || tup.n_chi_id_source === null){
                tableau_champs.push( '`chi_id_source` = NULL' );
            }else{
                if(isNaN(parseInt( tup.n_chi_id_source , 10 ))){
                    return({"__xst" : __xer ,"__xme" : 'le champ "id" doit être numérique'});
                }
                tableau_champs.push( '`chi_id_source` = ' + this.__ig1.__fnt1.sq0( tup.n_chi_id_source , 'n_chi_id_source' ) + '' );
            }
