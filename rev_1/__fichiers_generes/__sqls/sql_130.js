    ...
    */
    async sql( par , donnees_retournees ){
        ...
        if(par.hasOwnProperty( 'T0_chi_id_groupe' ) && par['T0_chi_id_groupe'] !== ''){
            where0+='\r\n' + this.__gi1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chi_id_groupe`' , par['T0_chi_id_groupe'] );
        }
        if(par.hasOwnProperty( 'T0_chp_nom_groupe' ) && par['T0_chp_nom_groupe'] !== ''){
            where0+=` AND \`T0\`.\`chp_nom_groupe\` LIKE ` + this.__gi1.__fnt1.sq2( par['T0_chp_nom_groupe'] ) + '\r\n';
        }
        if(par.hasOwnProperty( 'T0_chx_parent_groupe' ) && par['T0_chx_parent_groupe'] !== ''){
            where0+='\r\n' + this.__gi1.__fnt1.construction_where_sql_sur_id1( '`T0`.`chx_parent_groupe`' , par['T0_chx_parent_groupe'] );
        }
        ...
        sql0+=plage0;
        /* this.__gi1.ma_trace1('sql_130 sql0=',sql0); */
        let lignes=[];
        try{
        ...
        for(let numero_de_ligne in lignes){
            donnees0.push( {
                    "T0.chi_id_groupe" : lignes[numero_de_ligne][0] ,
                    "T0.chp_nom_groupe" : lignes[numero_de_ligne][1] ,
                    "T0.chx_parent_groupe" : lignes[numero_de_ligne][2] ,
                    "T1.chp_nom_groupe" : lignes[numero_de_ligne][3]
                } );
        }
        /* comptage */
