<?php
function sql_334($par,&$donnees_retournees,$that){
    $sql0='UPDATE `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.`tbl_genres` SET '.PHP_EOL;
    $tableau_champs=array();

    if($par['n_chi_id_genre']==='' || is_null($par['n_chi_id_genre']) ){
        $tableau_champs[]='`chi_id_genre` = NULL';
    }else{
        $tableau_champs[]='`chi_id_genre` = '.sq0($par['n_chi_id_genre']).'';
    }
