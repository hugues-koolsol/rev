...
function sql_115($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_115</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chp_nom_source']) && $par['T0_chp_nom_source'] !== '')){
        $where0.=' AND `T0`.`chp_nom_source` LIKE '.sq2($par['T0_chp_nom_source']).''.PHP_EOL;
    }
    if(( isset($par['T0_chi_id_source']) && $par['T0_chi_id_source'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_source`',$par['T0_chi_id_source']);
    }
    if(( isset($par['T0_chi_id_source2']) && $par['T0_chi_id_source2'] !== '')){
        $where0.=' AND `T0`.`chi_id_source` > '.sq1($par['T0_chi_id_source2']).''.PHP_EOL;
    }
    if(( isset($par['T0_chi_id_source3']) && $par['T0_chi_id_source3'] !== '')){
        $where0.=' AND `T0`.`chi_id_source` <= '.sq1($par['T0_chi_id_source3']).''.PHP_EOL;
    }
    if(( isset($par['T0_che_binaire_source']) && $par['T0_che_binaire_source'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_binaire_source`',$par['T0_che_binaire_source']);
    }
    if(( isset($par['T0_chx_dossier_id_source']) && $par['T0_chx_dossier_id_source'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chx_dossier_id_source`',$par['T0_chx_dossier_id_source']);
    }
    if(( isset($par['T1_chp_nom_dossier']) && $par['T1_chp_nom_dossier'] !== '')){
        $where0.=' AND `T1`.`chp_nom_dossier` LIKE '.sq2($par['T1_chp_nom_dossier']).''.PHP_EOL;
    }
    if(( isset($par['T0_che_contient_version_source']) && $par['T0_che_contient_version_source'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_contient_version_source`',$par['T0_che_contient_version_source']);
    }
    if(( isset($par['T0_che_autorisation_globale_source']) && $par['T0_che_autorisation_globale_source'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_autorisation_globale_source`',$par['T0_che_autorisation_globale_source']);
    }
    $sql0.=$where0;
    $order0='
    ...
    try{
        ...
        while(($tab0=$res0->fetchArray(SQLITE3_NUM))){
            $donnees0[]=array(
                ...
                'T1.chp_nom_dossier' => $tab0[6],
                'T0.che_binaire_source' => $tab0[7],
                'T0.che_contient_version_source' => $tab0[8],
                'T0.che_autorisation_globale_source' => $tab0[9],
            );
        }
        $stmt0->close();
