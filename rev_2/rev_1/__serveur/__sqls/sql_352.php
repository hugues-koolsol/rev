<?php
function sql_352($par,&$donnees_retournees,$that){
    $sql0='
      DELETE FROM `'.$GLOBALS[__BDD][BDD_NUMERO_1][PREFIXE_BDD].'`.tbl_requetes
          WHERE `chi_id_requete` = '.sq1($par['chi_id_requete']).' ;
    ';
    ...
    if($dep>0){
        $donnees_retournees[__xsi][__xer][]=' erreur lors de la suppression, il existe des dépendances sql_352() [' . __LINE__ . ']';
        return array(
            __xst => __xer,
            'sql0' => $sql0,
