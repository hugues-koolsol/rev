...
function sql_124($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_124</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chi_id_metier']) && $par['T0_chi_id_metier'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_metier`',$par['T0_chi_id_metier']);
    }
    if(( isset($par['T0_chp_nom_metier']) && $par['T0_chp_nom_metier'] !== '')){
    ...
    try{
        ...
        return array(
                __xst => __xer , 
                'sql0' => $sql0 ,
                'texte_requete' => 'la liste sur les metiers' ,
                'exception' => $e ,
                'id_bdd' => BDD_NUMERO_1 
            );
    }
