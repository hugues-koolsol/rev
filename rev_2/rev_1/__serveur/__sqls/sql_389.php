<?php
function sql_389($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_389</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chi_id_dossier']) && $par['T0_chi_id_dossier'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_dossier`',$par['T0_chi_id_dossier']);
    }
    if(( isset($par['T0_chp_nom_dossier']) && $par['T0_chp_nom_dossier'] !== '')){
        $where0.=' AND `T0`.`chp_nom_dossier` LIKE '.sq2($par['T0_chp_nom_dossier']).''.PHP_EOL;
    }
    if(( isset($par['T0_che_contient_genere_dossier']) && $par['T0_che_contient_genere_dossier'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_contient_genere_dossier`',$par['T0_che_contient_genere_dossier']);
    }
    if(( isset($par['T0_chx_parent_dossier']) && $par['T0_chx_parent_dossier'] !== '')){
    ...
    if(( isset($par['T1_chp_nom_dossier']) && $par['T1_chp_nom_dossier'] !== '')){
        $where0.=' AND `T1`.`chp_nom_dossier` = '.sq1($par['T1_chp_nom_dossier']).''.PHP_EOL;
    }
    if(( isset($par['T0_che_pour_les_js_dossier']) && $par['T0_che_pour_les_js_dossier'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_pour_les_js_dossier`',$par['T0_che_pour_les_js_dossier']);
    }
    $sql0.=$where0;
    $order0='
    ...
    try{
        ...
        while(($tab0=$res0->fetchArray(SQLITE3_NUM))){
            $donnees0[]=array(
                ...
                'T0.che_pour_les_js_dossier' => $tab0[5],
            );
        }
        $stmt0->close();
