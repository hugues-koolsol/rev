...
function sql_163($par,&$donnees_retournees,$that){
    ...
    if($par['n_che_contient_version_source']==='' || is_null($par['n_che_contient_version_source']) ){
        $tableau_champs[]='`che_contient_version_source` = NULL';
    }else{
        $tableau_champs[]='`che_contient_version_source` = '.sq0($par['n_che_contient_version_source']).'';
    }
    if($par['n_che_autorisation_globale_source']==='' || is_null($par['n_che_autorisation_globale_source']) ){
        $tableau_champs[]='`che_autorisation_globale_source` = NULL';
    }else{
        $tableau_champs[]='`che_autorisation_globale_source` = '.sq0($par['n_che_autorisation_globale_source']).'';
    }
