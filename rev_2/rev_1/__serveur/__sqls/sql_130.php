...
function sql_130($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_130</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chi_id_groupe']) && $par['T0_chi_id_groupe'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_groupe`',$par['T0_chi_id_groupe']);
    }
    if(( isset($par['T0_chp_nom_groupe']) && $par['T0_chp_nom_groupe'] !== '')){
        $where0.=' AND `T0`.`chp_nom_groupe` LIKE '.sq2($par['T0_chp_nom_groupe']).''.PHP_EOL;
    }
    if(( isset($par['T0_chx_parent_groupe']) && $par['T0_chx_parent_groupe'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chx_parent_groupe`',$par['T0_chx_parent_groupe']);
    }
    ...
    try{
        ...
        while(($tab0=$res0->fetchArray(SQLITE3_NUM))){
            $donnees0[]=array(
                ...
                'T1.chp_nom_groupe' => $tab0[2],
                'T0.chx_parent_groupe' => $tab0[3],
            );
        }
        $stmt0->close();
