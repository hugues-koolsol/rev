<?php
function sql_145($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_145</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chi_id_menu']) && $par['T0_chi_id_menu'] !== '')){
        $where0.=' AND `T0`.`chi_id_menu` LIKE '.sq2($par['T0_chi_id_menu']).''.PHP_EOL;
    }
    if(( isset($par['T0_cht_libelle_menu']) && $par['T0_cht_libelle_menu'] !== '')){
        $where0.=' AND `T0`.`cht_libelle_menu` LIKE '.sq2($par['T0_cht_libelle_menu']).''.PHP_EOL;
    }
    if(( isset($par['T0_chp_titre_menu']) && $par['T0_chp_titre_menu'] !== '')){
        $where0.=' AND `T0`.`chp_titre_menu` LIKE '.sq2($par['T0_chp_titre_menu']).''.PHP_EOL;
    }
    if(( isset($par['T0_chx_autorisation_menu']) && $par['T0_chx_autorisation_menu'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chx_autorisation_menu`',$par['T0_chx_autorisation_menu']);
    }
    if(( isset($par['T1_chp_nom_autorisation']) && $par['T1_chp_nom_autorisation'] !== '')){
        $where0.=' AND `T1`.`chp_nom_autorisation` = '.sq1($par['T1_chp_nom_autorisation']).''.PHP_EOL;
    }
    if(( isset($par['T0_chp_methode_menu']) && $par['T0_chp_methode_menu'] !== '')){
        $where0.=' AND `T0`.`chp_methode_menu` LIKE '.sq2($par['T0_chp_methode_menu']).''.PHP_EOL;
    }
    if(( isset($par['T1_chx_acces_autorisation']) && $par['T1_chx_acces_autorisation'] !== '')){
