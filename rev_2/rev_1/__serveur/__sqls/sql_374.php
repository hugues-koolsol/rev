<?php
function sql_374($par,&$donnees_retournees,$that){
    ...
    if(( isset($par['T0_chi_id_projet']) && $par['T0_chi_id_projet'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_projet`',$par['T0_chi_id_projet']);
    }
    if(( isset($par['T0_chp_nom_projet']) && $par['T0_chp_nom_projet'] !== '')){
        $where0.=' AND `T0`.`chp_nom_projet` LIKE '.sq2($par['T0_chp_nom_projet']).''.PHP_EOL;
    }
    if(( isset($par['T0_cht_commentaire_projet']) && $par['T0_cht_commentaire_projet'] !== '')){
