...
function sql_328($par,&$donnees_retournees,$that){
    if($GLOBALS[DEVER_SRV]>=2){
        $donnees_retournees[__xsi][__xif][]='info <b>sql_328</b>';
    }
    $champs0='
    ...
    if(( isset($par['T0_chi_id_genre']) && $par['T0_chi_id_genre'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`chi_id_genre`',$par['T0_chi_id_genre']);
    }
    if(( isset($par['T0_chp_nom_genre']) && $par['T0_chp_nom_genre'] !== '')){
        $where0.=' AND `T0`.`chp_nom_genre` LIKE '.sq2($par['T0_chp_nom_genre']).''.PHP_EOL;
    }
    if(( isset($par['T0_chp_prefixe_genre']) && $par['T0_chp_prefixe_genre'] !== '')){
        $where0.=' AND `T0`.`chp_prefixe_genre` LIKE '.sq2($par['T0_chp_prefixe_genre']).''.PHP_EOL;
    }
    if(( isset($par['T0_chp_espece_genre']) && $par['T0_chp_espece_genre'] !== '')){
        $where0.=' AND `T0`.`chp_espece_genre` LIKE '.sq2($par['T0_chp_espece_genre']).''.PHP_EOL;
    }
    if(( isset($par['T0_cht_valeur_init_genre']) && $par['T0_cht_valeur_init_genre'] !== '')){
        $where0.=' AND `T0`.`cht_valeur_init_genre` LIKE '.sq2($par['T0_cht_valeur_init_genre']).''.PHP_EOL;
    }
    if(( isset($par['T0_cht_parmis_genre']) && $par['T0_cht_parmis_genre'] !== '')){
        $where0.=' AND `T0`.`cht_parmis_genre` LIKE '.sq2($par['T0_cht_parmis_genre']).''.PHP_EOL;
    }
    if(( isset($par['T0_che_ordre_genre']) && $par['T0_che_ordre_genre'] !== '')){
        $where0.=PHP_EOL.construction_where_sql_sur_id1('`T0`.`che_ordre_genre`',$par['T0_che_ordre_genre']);
    }
    $sql0.=$where0;
    $order0='
